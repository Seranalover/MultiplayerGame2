...
ACPlayerCharacter::ACPlayerCharacter()
{
	...
	InventoryComponent = CreateDefaultSubobject<UInventoryComponent>("Inventory Component"); //库存（背包）组件
}
...
void ACPlayerCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (EnhancedInputComponent)
	{
		...
		EnhancedInputComponent->BindAction(LearnAbilityLeaderAction, ETriggerEvent::Started, this, &ACPlayerCharacter::LearnAbilityLeaderDown); //学习技能IA
		EnhancedInputComponent->BindAction(LearnAbilityLeaderAction, ETriggerEvent::Completed, this, &ACPlayerCharacter::LearnAbilityLeaderUp); //完成学习技能IA
...
void ACPlayerCharacter::LerpCameraToLocalOffsetLocation(const FVector& Goal)
{
	...
	CameraLerpTimerHandle = GetWorldTimerManager().SetTimerForNextTick(FTimerDelegate::CreateUObject(this, & ACPlayerCharacter::TickCameraLocalOffsetLerp, Goal)); //在下一次tick时创建插值计时器
}
